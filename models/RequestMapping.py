...
class RequestMapping(models.Model):
    ...
    from_manager = fields.Many2one('res.users','Manager:')
    to_confirmed_by = fields.Many2one('res.users','Accountant:')
    to_manager = fields.Many2one('res.users','Manager:')
    ...
    def init(self):
        ...
        self._cr.execute(""" CREATE OR REPLACE VIEW cash_managment_requestmapping AS
             (select row_number() OVER (ORDER BY 1) AS id,x.from_branch,x.to_branch,  x.confirmed_by AS from_confirmed_by,x.from_manager AS from_manager,y.confirmed_by AS to_confirmed_by, x.to_branch_manager AS to_manager ,x.confirm_date AS from_confirm_date, y.confirm_date AS to_confirm_date,x.total AS from_total,y.total AS to_total,x.total_usd AS from_total_usd,y.total_usd AS to_total_usd from cash_managment_request_confirmation x left join cash_managment_confirm_cash_to y on x.id = y.amount_request_id)""")
